...
static volatile int StoringData = 0;    // if this flag = 1, currently storing
                                        // plot data
static volatile float Kp = 0.0, Ki = 0.0;   // control gains
...
void __ISR(_TIMER_2_VECTOR, IPL5SOFT) Controller(void) {
    ...
    OC1RS = (unsigned int) ((unew/100.0)* PR3); //Use new values 

    adcval = adc_sample_convert(1);

    e = Waveform[counter] - adcval;
    Eint = Eint + e; // discrete integral is (running sum)
    u = Kp * e + Ki * Eint; // multiply kp by error, ki by its integral
...
void ADC_Startup() {
    ANSELAbits.ANSA1 = 1; // AN1 is adc pin
    AD1CON1bits.SSRC = 0x7; // Conversion process automatic
    AD1CON1bits.ASAM = 0;   // Sampling process manual
    AD1CON3bits.ADCS = 1;   // Set Tad to be 2*(ADCS + 1)* Tpb = 83 ns
    AD1CON3bits.SAMC = 30;  //set sampling time to be 20*Tad =
